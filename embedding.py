...
class Embedding:
    ...
    def build_model(self):
        if self.method == "word2vec":
            ...
            self.model, self.weights = get_weights_using_raw_networks(
                self.token_to_id, 30
            )
        elif self.method == "keras":
        ...
        self.embedding_matrix = np.zeros(
            (vocab_size, len(self.embeddings_index["def"]))
        )
...
def get_weights_using_word2vec(code_fragments):
    ...
    t = time()
    w2v_model.train(
        code_fragments, total_examples=w2v_model.corpus_count, epochs=30, report_delay=1
    )
...
def get_weights_using_keras(token_to_id, code_fragments):
    ...
    embedding_layer = layers.Embedding(
        N_WORDS, EMBEDDING_DIM, embeddings_initializer="RandomNormal", input_shape=(1,)
    )
